...
QCPingTest::QCPingTest(QObject *parent) : QObject(parent)
{
...
int QCPingTest::ping(bool bStart)
{
    if (true == bStart) {
        start();
    }
...
void QCPingTest::setIP(QString ip)
{
    m_IP = ip;
}

QString QCPingTest::getIP()
{
    return m_IP;
}

int QCPingTest::start()
{
    if (nullptr == m_pProcessPing) {
        return -1;
    }
...
void QCPingTest::processStarted()
{
    qDebug() << "processStarted()";
}

bool QCPingTest::isWinEnOutputString(QString s, QString &r)
{
    return false;
}

bool QCPingTest::isWinKrOutputString(QString s, QString &r)
{
    // 127.0.0.1의 응답: 바이트=32 시간<1ms TTL=128
    // 172.217.31.132의 응답: 바이트=32 시간=64ms TTL=113
...
bool QCPingTest::isLinuxEnOutputString(QString s, QString &r)
{
    // 64 bytes from 127.0.0.1: icmp_seq=1 ttl=64 time=0.099 ms
...
bool QCPingTest::checkOutputString(QString s, QString &r)
{
    QString tmp = "";
...
QStringList QCPingTest::convToLineByLine(QString s)
{
...
qlonglong QCPingTest::getResponseTime(QString s)
{
    // =123
    // <1
...
void QCPingTest::readyReadStandardOutput()
{
...
void QCPingTest::processFinished()
{
